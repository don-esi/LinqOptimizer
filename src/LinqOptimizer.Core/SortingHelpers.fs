                    ...
                    let rec loopArray loopIdx actionIdx loopHigh =
                        if loopIdx < iterations then
                            if (actionIdx % 2 = 1) then
                                ...
                                let (fromArr, fromKeys) = getMergeArray FromArrayType
                                let (toArr, toKeys) = getMergeArray ToArrayType
                                merge fromArr fromKeys toArr toKeys low loopHigh (loopHigh + 1) newHigh
                                barrier.SignalAndWait()
